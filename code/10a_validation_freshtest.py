...
np.random.seed(20260615)  # distinct seed so it's a genuinely new draw, not the same RNG path
SRC=COLLAB+"/data/corpus_with_topics.parquet"
KEYS={78,79}; N=60
# exclude original 1500 sentences
prev=pd.read_csv(OUT+"/stance_train_1500_KEY.csv")
excl=set(zip(prev["show"],prev["episode_number"],prev["sentence_id"]))
print("excluding",len(excl),"original rows",flush=True)
keep=[]
...
print("fresh candidate pool:",len(cand),flush=True)
s=cand.sample(N,random_state=20260615).reset_index(drop=True)
# neighbor context
...
s=s.sample(frac=1,random_state=20260615).reset_index(drop=True)
s.insert(0,"test_id",["T"+str(i+1).zfill(3) for i in range(len(s))])
s["russia_stance"]=""; s["ukraine_stance"]=""; s["notes"]=""
s[["test_id","prev_sentence","sentence","next_sentence","russia_stance","ukraine_stance","notes"]].to_csv(OUT+"/stance_freshtest_60.csv",index=False)
s[["test_id","show","date","episode_number","sentence_id","topic"]].to_csv(OUT+"/stance_freshtest_60_KEY.csv",index=False)
print("WROTE stance_freshtest_60.csv (",len(s)," rows ); topic dist:",dict(s["topic"].value_counts()),flush=True)
print("DONE_FRESH",flush=True)
