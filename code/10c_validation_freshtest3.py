...
np.random.seed(20260615+3)
SRC=COLLAB+"/data/corpus_with_topics.parquet"
KEYS={78,79}; N=60
excl=set()
for fn in ["stance_train_1500_KEY.csv","stance_freshtest_60_KEY.csv","stance_freshtest2_60_KEY.csv"]:
    p=pd.read_csv(OUT+"/"+fn); excl|=set(zip(p["show"],p["episode_number"],p["sentence_id"]))
...
print("fresh3 pool:",len(cand),flush=True)
s=cand.sample(N,random_state=20260618).reset_index(drop=True)
...
s=s.sample(frac=1,random_state=20260618).reset_index(drop=True)
s.insert(0,"test_id",["V"+str(i+1).zfill(3) for i in range(len(s))])
s["russia_stance"]=""; s["ukraine_stance"]=""; s["notes"]=""
s[["test_id","prev_sentence","sentence","next_sentence","russia_stance","ukraine_stance","notes"]].to_csv(OUT+"/stance_freshtest3_60.csv",index=False)
s[["test_id","show","date","episode_number","sentence_id","topic"]].to_csv(OUT+"/stance_freshtest3_60_KEY.csv",index=False)
print("WROTE stance_freshtest3_60.csv (",len(s),"rows ); topic:",dict(s["topic"].value_counts()),flush=True)
print("DONE_FRESH3",flush=True)
